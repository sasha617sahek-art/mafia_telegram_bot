...
async def game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    games[chat_id] = {"players": {}, "alive": [], "state": "lobby", "night": 0, "muted": set()}
...
async def resolve_night(chat_id, context):
    ...
    elif action in ["kiss", "hug"]:
        text += "🌙 Ночь прошла тихо...\n"

    else:
        text += "🌙 Ночь прошла без событий...\n"

    # Добавляем список живых игроков
    alive_names = []
...
async def end_game(chat_id, context):
    ...
    if len(game["alive"]) == 1:
        winner = game["alive"][0]
        winner_name = game["players"].get(winner, str(winner))
        await context.bot.send_message(chat_id, f"🏆 Победитель: {winner_name}")
    else:
        await context.bot.send_message(chat_id, "Никто не выжил...")
    del games[chat_id]
