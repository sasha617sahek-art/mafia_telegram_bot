...
fuel_prices = {"diesel": 5, "gas92": 4, "gas95": 6}
    ...
    for fuel in ["diesel", "gas92", "gas95"]:
        for level in ["мини", "среднее", "большое"]:
            buttons.append([InlineKeyboardButton(
                f"{fuel.capitalize()} {level} ({equipment_cost[level]}💰)",
                callback_data=f"buy_{fuel}_{level}"
            )])
    ...
    if data.startswith("buy_"):
        ...
        if p['equipment'][fuel]:
            await query.edit_message_text(
                f"У тебя уже есть оборудование для {fuel} ({p['equipment'][fuel]}). Продай его, чтобы купить новое."
            )
            return
        ...
        await query.edit_message_text(f"Ты купил {fuel} оборудование {level}!")
...
async def pay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    p['money'] -= amount
    target['money'] += amount
    await update.message.reply_text(f"Ты перевёл {amount}💰 игроку {target_nick}!")
...
async def sell_equipment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Ты продал оборудование {fuel} {level} за {sell_price}💰")
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        ...
        "/upgrade_well — прокачать скважину\n"
        "/sell_eq <топливо> — продать оборудование\n"
